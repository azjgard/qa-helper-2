(function($, global) {
  ...
  var templateObjects = {
    ...
    "old-slide": {
      ...
      buttons : [
        ...
        {
          text   : '<input type="text" placeholder="Paste CC text here..." id="qa-ext_compare_cc_on_page" />',
          id     : 'qa-ext_compare_cc',
          classes: ['button-icon-sibling'],
          listener: {
            func : compareCC,
            type : 'paste'
  ...
  // compareCC
  //
  // descr - pulls up the help page for the Courseware Tool (/help.html)
  function compareCC(e) {
    setTimeout(function(){
      var input = document.querySelector('#qa-ext_compare_cc_on_page');
  ...
  // descr - pulls up the help page for the Courseware Tool (/help.html)
  function helpPage() {
    chrome.runtime.sendMessage({message: 'help-page', data: null});
  }
      ...
      if (context !== 'misc' && templateObjects[context] !== undefined) {
        ...
        for (var i = 0; i < template.buttons.length; i++) {
          ...
          if (btn.id && btn.listener !== undefined) {
            $('#' + btn.id).on(btn.listener.type, btn.listener.func);
          }
          else {
            if (!btn.id) {
  ...
  //            type : type of listener (e.g. click)
  //          }
  //          popup: boolean,
  ...
  global.generateTemplate = function(templateObject) {
    ...
    //add help button
    str += '<button id="qa_ext-help-button">?</button>';
